...
**UUID**（Universally Unique Identifier，通用唯一标识符）是一种用来生成**全球唯一的 ID**的方式，由 32 个十六进制字符组成，分成五段（8-4-4-4-12）

**Request ID** 是一个专门用来标识一次 HTTP 请求的唯一 ID，通常就是用 UUID 来生成的

- 每个请求都有一个独立的 ID，便于追踪
- 如果系统出错，开发者可以根据这个 ID 找到相关日志
- 在微服务架构中，可以跨多个服务传递这个 ID，实现链路追踪
- 有助于调试、性能分析、错误定位

**X-Request-ID**是一个HTTP 请求头字段，用来传递 Request ID

> 客户端设置 `X-Request-ID` 的原因：
>
> 1. **端到端追踪**
>
> 客户端（比如浏览器、移动 App、API 调用方）可以生成一个唯一的 ID，并在请求头中带上它。这样：
>
> - 客户端可以记录这个 ID，用于后续日志或错误报告
> - 服务端会将这个 ID写入日志，便于开发者查找
> - 如果请求跨多个服务（比如微服务架构），这个 ID 可以在整个链路中传递
>
> > 举个例子：你用手机 App 调用一个 API，App 生成 `X-Request-ID: abc-123`，服务端收到后就知道这是你这次请求的标识符，日志里也会记录它。
>
> 2. **幂等性控制（Idempotency）**
>
> 对于一些敏感操作（比如支付、创建订单），客户端可以设置一个固定的 `X-Request-ID`，确保即使请求重试，也不会重复执行。
>
> > 比如你点击“支付”按钮，网络卡顿导致你点了两次，但请求头中都带着同一个 `X-Request-ID`，服务端就能识别这是同一个操作，只处理一次。
>
> 3. **调试和错误报告**
>
> 如果客户端遇到错误，可以把 `X-Request-ID` 发给技术支持，开发者就能快速在日志中定位问题。
